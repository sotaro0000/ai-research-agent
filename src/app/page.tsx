  ...
  return (
    <div className="min-h-screen">
      ...
      <header className="border-b border-stone-300/70">
        <div className="mx-auto flex max-w-4xl items-center justify-between px-5 py-3.5">
          <div className="flex items-baseline gap-2.5">
            ...
            <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-stone-400">dossier</span>
          </div>
          <a
      ...
      <main className="mx-auto max-w-4xl px-5 py-12 sm:py-16">
        ...
        <div className="max-w-2xl">
          ...
          <form onSubmit={onSubmit} className="mt-7 space-y-2.5">
            ...
            {/* APIキー（BYOK） */}
            <div className="border border-stone-300/70 bg-white">
              <button
                type="button"
              ...
              >
                ...
                <span className="font-mono text-[11px] text-stone-400">{keysOpen ? "− close" : "+ open"}</span>
              </button>
              {keysOpen && (
                <div className="space-y-3 border-t border-stone-100 p-3.5">
                  <p className="border-l-2 border-stone-300 bg-stone-50 py-1.5 pl-2.5 text-xs leading-relaxed text-stone-500">
                    ご自身の API キーを入力すると本物のWeb調査が有効になります。
                    <strong className="text-stone-700">キーはこのリクエストの実行にのみ使用し、サーバーには保存しません。</strong>
                  </p>
        ...
        <div className="mt-12">
          ...
          {loading && (
            <div className="space-y-px bg-stone-200">
              ...
              <p className="bg-paper pt-3 text-center font-mono text-[11px] text-stone-400">
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 bg-white px-5 py-3">
                <div>
                  ...
                  <p className="font-display text-base font-semibold text-stone-900">
                    {result.report.topic}
                    <span className="ml-2 font-sans text-xs font-normal text-stone-400">／ {result.report.focus}</span>
                  </p>
                </div>
                <div className="text-right">
                  <p className={`font-mono text-[11px] uppercase ${MODE_BADGE[result.mode] ?? "text-stone-400"}`}>
                    {result.modeLabel}
                  </p>
                  <p className="font-mono text-[11px] text-stone-400">{result.elapsedMs}ms</p>
                </div>
              </div>
      ...
      <footer className="mx-auto max-w-4xl px-5 pb-12 pt-6">
        <div className="border-t border-stone-300/70 pt-5 font-mono text-[11px] leading-relaxed text-stone-400">
