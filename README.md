...
このアプリは **BYOK（Bring Your Own Key）** 方式です。運営側でキーを用意せず、**利用者が画面の「APIキー設定」に自分の OpenAI / Tavily キーを入力**して使います。

- 入力されたキーは **そのリクエストの実行にのみ使用し、サーバーには保存・記録しません**。
- 「このブラウザに保存」を選んだ場合のみ、**利用者のブラウザ内（localStorage）にのみ**保存されます。
- 環境変数（`OPENAI_API_KEY` 等）は、運営側で共通キーを使いたい場合の**任意のフォールバック**としてのみ機能します。
